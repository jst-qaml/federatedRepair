...
class RepairModel(metaclass=ABCMeta):
    """Meta class of model."""
    ...
    @abstractmethod
    def compile(self, input_shape, output_shape):  # noqa: A003
...
def train(
    ...
    # Load Model
    try:
        _model: tf.keras.Model = model.compile(
            dataset.image_shape[1:], dataset.label_shape[1]
        )
    except IndexError:
...
def test(model_dir: Path, data_dir: Path, target_data: str, batch_size: int = 32):
    ...
    score = model.evaluate(images, labels, verbose=0, batch_size=batch_size)
...
def _save_negative_results(results, data_dir: Path, path):
    ...
    for test_label in results:
        ...
        images_per_test_label, labels_per_test_label = _create_merged_dataset(
            results[test_label]
        )
...
def load_model_from_h5(model_dir: Path):
    """
...
def load_model_from_tf(model_dir: Path):
    ...
    if "hydra" not in str(model_dir):
        model = tf.keras.models.load_model(model_dir)
    else:
        model = load_model_from_h5(model_dir)
