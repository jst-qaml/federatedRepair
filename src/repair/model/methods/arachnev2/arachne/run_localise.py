...
import repair.methods.arachnev2.arachne.lstm_layer
import repair.methods.arachnev2.arachne.utils.model_util as model_util
...
def compute_gradient_to_loss(path_to_keras_model, idx_to_target_layer, X, y, 
    ...
    if loss_func == 'categorical_cross_entropy':
        ...
        #	logits = model.output,
        #	name = "per_label_loss")
        loss_tensor = tf.keras.losses.CategoricalCrossentropy()(y_tensor,model.output)
...
def compute_FI_and_GL(
    ...
    for idx_to_tl, vs in target_weights.items():
        ...
        if model_util.is_FC(lname):
            ...
            from_behind = compute_gradient_to_output(path_to_keras_model, idx_to_tl, target_X,federated=federated)
            ...
            from_behind = compute_gradient_to_output(path_to_keras_model, idx_to_tl, target_X, by_batch = False,federated=federated)
